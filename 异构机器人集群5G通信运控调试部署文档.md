...
本文汇总 2026-08-09 至 2026-08-21 的异构机器人集群通信、运控、部署和故障排查记录，
...
| M4T 5G/ECS | 已实现 STARTUP、单目标 NAVIGATE、CANCEL、RETURN_HOME、持久化和安全门禁；ECS 与妙算 DPK 已部署；试验参数为 2m/1m/s | DJI Assistant 权限与“度”单位已确认；导航开关已启用；待下次上电回读和首次受控实飞；HTTPS 与 Home 高度遥测命名问题仍未收口 |
...
固定端侧限制为：最低航路高度 2m、水平速度 1m/s（PSDK 该字段可表达的最小值）、首次 Home 半径 100m、
目标相对原始地面椭球高 2-30m、电量至少 10%、GPS Fix 3/4、至少 12 星、水平/垂直精度不超过 2m/3m、
...
2026-08-21 实飞准备参数已设为：默认目标纬度 `22.604375789`、经度
`114.057071644`、WGS84 椭球高 `106.0m`。这不会自动发送命令，只有显式执行
`run` 才会提交。`2m` 是 PSDK `mea` 最低航路高度，不是最终目标的相对高度。
目标相对高度按“目标椭球高 - 地面 STARTUP 时 `POSITION_FUSED.altitude`”计算。

PSDK `TOPIC_ALTITUDE_OF_HOMEPOINT` 是 ICAO/气压高度，不是 WGS84 椭球高。2026-08-21
已修复状态机，不再用该值做目标高度门禁。为保持当前 ECS 严格 schema 兼容，遥测键
`home.altitude_ellipsoid_m` 暂时仍携带气压高度，不得用于航高计算。
...
2026-08-21 的实飞参数准备和重新部署记录如下：

- PC 默认目标更新为 `22.604375789, 114.057071644, 106.0m`；妙算最低航路高度为 `2m`、水平速度为 `1m/s`、最低目标相对高度为 `2m`。
- 发现 `TOPIC_ALTITUDE_OF_HOMEPOINT` 气压高度约 `204m`，而 `POSITION_FUSED.altitude` 椭球高约 `101m`；已修复地面 STARTUP，使用后者建立原始地面椭球高基准。
- 妙算原生构建和同版本 DPK 更新成功。最终已安装二进制 SHA-256 为
  `0269e8293402e63753f52dcbef673a35d54299267a6baf7694695ab68d5526d6`，DPK SHA-256 为
  `3bec79d22be7a080185fd7a1b339abfa38bbf84b15be4104f07150a724b6f84f`。
- 安装后 `airs-5g-relay` 为 `Running`、ECS 在线、`psdk_connected=true`、飞行状态 `STOPPED`、任务 `active=false`；电量 63%、GPS Fix 3、24 星、三向避障开启。
- `navigation.enabled=false`、`coordinate_units_verified=false`，状态 journal 不存在；全过程未发送 STARTUP、NAVIGATE、CANCEL 或 RETURN_HOME。
- 如下一步在目前地面椭球高约 `100.93m` 时执行 STARTUP，目标 `106m` 的相对高度约为 `5.07m`，不是 `2m`。

2026-08-21 首次实飞前的继续准备记录：

- 用户明确将程序最低导航电量门禁从 `50%` 调整为 `10%`；DJI 飞控自身低电量保护不变。
- 已确认 DJI Assistant 高级 Flight Control 权限和经纬度“度”单位，妙算配置中
  `navigation.enabled=true` 且 `coordinate_units_verified=true`。
- 一次误用过期的 `native-build/source` 目录构建，启动日志明确显示
  `flight-control commands are disabled`；在下发任何 STARTUP/NAVIGATE 前已发现并立即回滚。
- 随后从包含完整 `m4t_navigation.c`/`m4t_navigation_core.c`/扩展遥测的
  `nav-20260820/source` 重新原生构建并安装成功。最终二进制 SHA-256 为
  `9b8cc4c465c6b760dac7da136cc18e1cb8985c6735ae381ff0c5642296ed59c4`，DPK SHA-256 为
  `bd47e6ac4158eb9645e9a9f1c56b7e4d9867f62beec1fdf7eb1f92c385f24197`。
- DPK 安装返回成功后无人机关机，因此还需在下次上电后回读 `Running`、已安装哈希、完整遥测和无活动任务；关机前全程未下发飞行命令。
- 外场客户端可执行 `./m4t_navigation_test/field_test.sh`，依次完成 STARTUP 和固定目标导航；
  STARTUP 失败会直接停止，不会提交 NAVIGATE。
- 操作者已明确授权承担明文 HTTP 飞行命令的风险。ECS 已备份 `/etc/m4t-relay.env`，持久设置
  `M4T_ALLOW_INSECURE_NAVIGATION=true` 并重启；服务为 `active`，进程环境已回读为 `true`，本机和公网 `/health` 均返回 `{"status":"ok"}`。
...
| 2026-08-21（M4T 实飞准备） | 更新试验目标、航高和速度，修复 Home 高度基准 | 默认目标为 `22.604375789, 114.057071644, 106m`，妙算固定参数为 `2m/1m/s`；新 DPK 已安装且 Relay 恢复在线，导航开关仍关闭，未发送任何飞行命令 |

...
#include "m4t_cloud_relay/test_m4t_cloud_relay.h"
...
#define DJI_SYSTEM_CMD_STR_MAX_SIZE     (DJI_LOG_PATH_MAX_SIZE + 64)
...
#ifdef CONFIG_MODULE_SAMPLE_M4T_CLOUD_RELAY_ON
    if (aircraftInfoBaseInfo.aircraftType != DJI_AIRCRAFT_TYPE_M4T) {
        USER_LOG_ERROR("M4T cloud relay test only supports Matrice 4T");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
    returnCode = DjiTest_M4tCloudRelayStartService();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("M4T cloud relay service init error");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
#endif
